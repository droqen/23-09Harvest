...
#[main]
pub fn main() {
    // camera_with_fog::init();
    // fog_example_default_camera::init();
    fog_example_ground_and_cubes::init(false, true);
}
...
mod fog_example_default_camera {
    use ambient_api::{
        core::{
            app::components::main_scene,
            camera::{
...
mod fog_example_ground_and_cubes {
    use ambient_api::{
        core::{
    ...
    pub fn init(ground: bool, cubes: bool) {
        if ground {
            Entity::new()
                .with_merge(make_transformable())
                .with(quad(), ())
...
mod fog_example_ui_sliders_for_sun {
    use ambient_api::{
